import {
  PERSONAL as FALLBACK_PERSONAL,
  SKILLS as FALLBACK_SKILLS,
  PROJECTS as FALLBACK_PROJECTS,
  JOURNEY as FALLBACK_JOURNEY,
 ...
 *
 * Now content-driven: pass the live content from the API (with the bundled
 * data as fallback) so answers stay in sync with the admin panel.
 */

export function buildGreeting(content = {}) {
  const PERSONAL = content.PERSONAL || FALLBACK_PERSONAL;
  return {
    text: `Hi! 👋 I'm ${PERSONAL.name?.split(" ")[0] || "the"}'s assistant. Ask me about his skills, projects, experience, or how to get in touch.`,
    chips: ["What can he build?", "Show projects", "Tech skills", "Hire / availability", "Contact"],
  };
}

// Static greeting kept for backwards compatibility.
export const GREETING = buildGreeting();

function buildIntents(content) {
  const PERSONAL = content.PERSONAL || FALLBACK_PERSONAL;
  const SKILLS = content.SKILLS?.length ? content.SKILLS : FALLBACK_SKILLS;
  const PROJECTS = content.PROJECTS?.length ? content.PROJECTS : FALLBACK_PROJECTS;
  const JOURNEY = content.JOURNEY?.length ? content.JOURNEY : FALLBACK_JOURNEY;
...
export function getBotResponse(input, content = {}) {
  const text = (input || "").toLowerCase().trim();
  const fb = fallbackResponse(content);
  if (!text) return fb;

  const intents = buildIntents(content);
  let best = null;
  let bestScore = 0;
  ...
  return best ? best.answer() : fb;
}

export const allSkills = FALLBACK_SKILLS.flatMap((g) => g.items.map((i) => i.name));
